      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card, i) => {
          ...
          return (
            <div
              key={i}
              className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 hover:shadow-lg transition-all duration-300 group"
            >
              <div className="flex items-start justify-between mb-4">
                <div className={`${card.lightBg} p-2.5 sm:p-3 rounded-xl`}>
                  <Icon
                    size={20}
                    sm:size={24}
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
            ...
            <div className="px-3 py-1.5 sm:px-4 sm:py-2 bg-slate-100 rounded-xl">
          ...
          <div className="h-[250px] sm:h-[320px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Bar dataKey="score" radius={[8, 8, 0, 0]} barSize={30}>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
          ...
          <div className="space-y-3 flex-1 overflow-y-auto max-h-[300px] sm:max-h-[350px]">
            {recentAssignments.map((assign, index) => (
              <div
                key={assign.id}
                onClick={() => navigate(`/admin/assignments/${assign.id}`)}
                className="group cursor-pointer p-3 sm:p-4 rounded-xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-200"
              >
            ...
            {recentAssignments.length === 0 && (
              <div className="flex flex-col items-center justify-center py-10 sm:py-12 opacity-40">
          ...
          {recentAssignments.length > 0 && (
            <button
              onClick={() => navigate("/admin/assignments")}
              className="w-full mt-4 py-2.5 sm:py-3 bg-slate-50 hover:bg-slate-100 rounded-xl text-sm font-semibold text-slate-700 transition-all"
            >
              Lihat Semua Tugas
            </button>
          )}
